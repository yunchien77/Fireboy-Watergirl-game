...
void RestrictPlayerPosition(Character& player) {
    ...
    if (pos.x < LEVEL_MIN_X) pos.x = LEVEL_MIN_X;
    if (pos.x > LEVEL_MAX_X) pos.x = LEVEL_MAX_X;
    if (pos.y < LEVEL_MIN_Y) pos.y = LEVEL_MIN_Y;
    if (pos.y > LEVEL_MAX_Y) pos.y = LEVEL_MAX_Y;
...
void App::GamePlay() {
    ...
    if (Util::Input::IsKeyPressed(Util::Keycode::UP)) fireboyMoveY = 5;
    if (Util::Input::IsKeyPressed(Util::Keycode::DOWN)) fireboyMoveY = -5;
    ...
    if (Util::Input::IsKeyPressed(Util::Keycode::W)) watergirlMoveY = 5;
    if (Util::Input::IsKeyPressed(Util::Keycode::S)) watergirlMoveY = -5;
