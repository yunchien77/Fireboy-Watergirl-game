...
Gem::Gem(GemType type)
    : m_Type(type), Util::GameObject(nullptr, 20.0f) // z-index 可調整
{
...
void Gem::OnCharacterEnter(Character* character) {
    ...
    if (SDL_HasIntersection(&rect, &character->getRect())) {
        if (m_Type == GemType::FIRE && dynamic_cast<Fireboy*>(character) == nullptr) {
            return;
        }

        if (m_Type == GemType::WATER && dynamic_cast<Watergirl*>(character) == nullptr) {
            return;
        }
...
void Gem::SetInitialPosition(const glm::vec2& pos) {
